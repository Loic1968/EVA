...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await signup(email.trim(), password, displayName.trim() || undefined, stayConnected);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-eva-panel rounded-xl border border-slate-200 dark:border-slate-700/40 p-6 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-slate-600 dark:text-slate-400 mb-1">Name (optional)</label>
              <input
                type="text"
                value={displayName}
