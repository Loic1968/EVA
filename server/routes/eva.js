...
// ════════════════════════════════════════════════════════════════
const EVA_CHAT_SYSTEM = `You are EVA, a helpful AI assistant made by HaliSoft.

Match the user's language (French by default). Be concise and natural.

The user's personal data (emails, documents, calendar) may appear below as ## sections — use them to answer. Never invent facts. If data is missing, say so briefly.`;
  ...
  try {
    ...
    if (!result && (process.env.EVA_ASSISTANT_MODE === 'true' || process.env.EVA_OVERHAUL_ENABLED === 'true') && req.ownerId) {
      try {
        ...
        console.warn('[EVA] preAnswer failed:', e.message);
      }
    }
    if (!result) {
